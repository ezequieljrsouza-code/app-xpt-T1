...
# --- 2. CONEXÃO COM FIREBASE (VERSÃO CORRIGIDA) ---
@st.cache_resource
def get_db():
    # O Streamlit já carrega o segredo como dicionário se estiver no formato TOML correto
    key_dict = st.secrets["firestore_key"]
    
    # Caso você tenha colado como string por engano, esse bloco resolve:
...
# --- 4. CALLBACKS PARA ATUALIZAÇÃO RÁPIDA ---
...
# --- 7. INICIALIZAÇÃO E ORGANIZAÇÃO DE DADOS ---
                ...
                with st.popover("🔄"):
                    for dest in st.session_state.dados_controle.keys():
                        if dest != rota and st.button(dest, key=f"mv_{rota}_{dest}_{idx}"):
                            ...
                            st.rerun()
            with c3:
                if st.button("❌", key=f"x_{rota}_{idx}"):
                    info['veiculos'].pop(idx)
                    salvar_no_firebase()
                    st.rerun()
            st.divider()
